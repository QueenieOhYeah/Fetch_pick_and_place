...
class AutoPickTeleop(object):
    ...
    def _pick_object(self):
        if self._poses_are_reachable:
            ...
            ps = PoseStamped()
            ps.header = self._i_marker.header
            ps.pose = copy.deepcopy(self._i_marker.pose)
            ps.pose.position.x += self._pregrasp_offset[0]
            ps.pose.position.y += self._pregrasp_offset[1]
            ps.pose.position.z += self._pregrasp_offset[2]
            self._arm.move_to_pose(ps)
            ...
            ps2.pose = copy.deepcopy(self._i_marker.pose)
            self._arm.move_to_pose(ps2)
            rospy.sleep(4)
            ...
            ps3.pose = copy.deepcopy(self._i_marker.pose)
            ps3.pose.position.x += self._lifted_offset[0]
            ps3.pose.position.y += self._lifted_offset[1]
            ps3.pose.position.z += self._lifted_offset[2]
            self._arm.move_to_pose(ps3)
            rospy.sleep(4)
